...
void updateVertices(int cube[54][3]) {  // the function receives as parameter a matrix formed by overlaping the 6 matrices corresponding 
	...
	for (int i = 0; i < 18; i++) { // number of rows = 18 (6 * 3)
		for (int j = 0; j < 3; j++) { // number of columns = 3
			GLfloat color[3] = { RGB[cube[i][j]][0], RGB[cube[i][j]][1], RGB[cube[i][j]][2] };
			vertices[i * j * VERTICES_ELEMENTS_FOR_ONE_CELL + 0 * 6 + 3] = color[0];
			vertices[i * j * VERTICES_ELEMENTS_FOR_ONE_CELL + 0 * 6 + 4] = color[1];
			vertices[i * j * VERTICES_ELEMENTS_FOR_ONE_CELL + 0 * 6 + 5] = color[2];
			vertices[i * j * VERTICES_ELEMENTS_FOR_ONE_CELL + 1 * 6 + 3] = color[0];
			vertices[i * j * VERTICES_ELEMENTS_FOR_ONE_CELL + 1 * 6 + 4] = color[1];
			vertices[i * j * VERTICES_ELEMENTS_FOR_ONE_CELL + 1 * 6 + 5] = color[2];
			vertices[i * j * VERTICES_ELEMENTS_FOR_ONE_CELL + 2 * 6 + 3] = color[0];
			vertices[i * j * VERTICES_ELEMENTS_FOR_ONE_CELL + 2 * 6 + 4] = color[1];
			vertices[i * j * VERTICES_ELEMENTS_FOR_ONE_CELL + 2 * 6 + 5] = color[2];
			vertices[i * j * VERTICES_ELEMENTS_FOR_ONE_CELL + 3 * 6 + 3] = color[0];
			vertices[i * j * VERTICES_ELEMENTS_FOR_ONE_CELL + 3 * 6 + 4] = color[1];
			vertices[i * j * VERTICES_ELEMENTS_FOR_ONE_CELL + 3 * 6 + 5] = color[2];
		}
	}
}
